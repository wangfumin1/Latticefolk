...
- multi-factor stability / generation-local evidence: se repite el ajuste eliminando una generación de la especie objetivo cada vez; solo los refits estimables con el mismo feature set que el modelo pooled entran en el rango de coeficientes y la consistencia de signo. Para cada generation endpoint reciente se amplía hacia atrás hasta encontrar la ventana más estrecha que cumple coverage / sample / variance / collinearity. Como la exposure persistida es lifetime aggregate, no se inventa una serie temporal diaria ni se sincronizan generaciones entre especies.
