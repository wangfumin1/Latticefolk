...
- phenotype→function trade-off: el phenotype heredado se convierte de forma determinista en multiplicadores estrechos de movement speed, locomotion energy, maintenance, forage efficiency, rest recovery y fast-action cost. Las patas largas pueden aumentar la velocidad pero también el coste de movimiento; un frame mayor aumenta el coste de mantenimiento. God View relaciona phenotype con reproduction / offspring / lifespan dentro del lifetime dominant biome realmente observado, usando right-censoring; `legacy_upgrade` sigue siendo descriptivo y no entra en evidencia histórica de adaptación.
- constrained organism families: la wildlife actual se agrupa en lagomorph / cervid / suiform / caprine / canid / mustelid / felid / bovid / procyonid. Cada fine individual mantiene un family genome con founder determinista e inheritance/mutation acotados para material palette, preferencias dentro de plant niches ya legales y el trade-off stride/endurance de velocidad frente a energía. El genome no puede habilitar un plant axis que sea cero para la especie, cambiar el prey graph, trophic role ni la ecología coarse. Jev solo lo recibe como contexto read-only. Fine state, transfers y lineage lo persisten, y God View/evolution API muestran provenance coverage, medias, varianza, tendencias por generación y breeder differential.
- reusable generated organism archetypes: un species profile se compone ahora de siete módulos simulation-owned: habitat, ecology/trophic niche, procedural body/family, movement mode, semantic wildlife capabilities, organism form y life-history. Además de lynx/bison/raccoon, sheep reutiliza open-plains + large-grazer + bovid como form `domesticated`, y warg reutiliza forest/hills + predator + canid como form `monster`. `wild/domesticated/monster` solo modifican de forma determinista la sensibilidad coarse a settlement/danger y se exponen a God View/Jev como contexto read-only. `domesticated` no implica owner, tame progress, comandos ni ownership del jugador.
