...
## Long-term quality gate: real E2E playtesting and visual validation

Latticefolk should not treat unit/integration tests and CI as sufficient proof that a playable world still works. As the project gains more simulation layers, every meaningful playable milestone should also be validated through a real end-to-end play session against a running build. The E2E path should exercise first-person entry and exploration, chunk discovery/streaming, coarse↔fine materialization/fold-back, NPC interaction, wildlife behavior and interaction, semantic WorldObject actions, production/inventory consequences, God View switching and inspection, save/reload persistence, and recovery of previously discovered world state. The test must verify actual player-observable outcomes rather than only calling isolated APIs.

Visual validation is a parallel requirement. Playable milestones should include rendered-scene inspection for procedural organisms/buildings, scale and footprint coherence, animation/controller behavior once available, terrain/road/object overlap, clipping, lighting/material regressions, camera framing, first-person readability, God View selection/path overlays, evolution/interaction panels, localization overflow, and obvious z-fighting or missing geometry. When automated browser/computer tooling can capture stable screenshots, representative scenes and UI states should become regression evidence; until then, explicit visual inspection remains part of acceptance.

These checks are quality gates, not new simulation authority. E2E tooling may observe and drive legal player input, but must not bypass deterministic rules to manufacture expected state. God View tests must preserve the hard observer invariant: camera motion must not explore/generate chunks, the player must not enter NPC perception or decision candidates, and stale player-targeting/dialogue responses must remain invalid after switching modes. Failures discovered through actual play or visual review should be fixed and re-run before a milestone is considered complete.
