...
## Implemented milestone: reusable generated organism archetypes

A species profile no longer has to be authored as one monolithic literal. `wildlifeArchetypes.ts` defines reusable habitat, ecology/trophic-niche, procedural-body/family, movement-mode, semantic action-capability and life-history modules plus a deterministic composer that produces an ordinary `WildlifeSpeciesProfile`. Family ownership now lives in the body/species archetype path, eliminating the former second species→family mapping; every downstream system still consumes the same registry.

Lynx validates a composed felid predator. Bison adds an open-plains large-grazer bovid with heavy movement and no hunt capability. Raccoon adds a forest/wetland-edge opportunistic procyonid omnivore with both forage and legal rabbit-hunt capability. The registry automatically gives each species coarse carrying capacity, seasonal suitability/migration, niche competition, disease transmission, predator pressure, fine materialization, reproduction, family-genome handling, lineage/evolution and interaction-network evidence. Reusable procedural features now include ear tufts, face masks and ringed tails, so none of these composed species requires a species-specific asset or simulation branch.

Movement/capability modules are now executable semantics. Fine locomotion composes species movement factors with phenotype physiology and inherited family-genome stride/endurance. The legal action set originates from the species capability module and is deterministically narrowed by age, pregnancy, migration state and predator graph before any provider sees it. This makes generated organisms semantic entities rather than generated meshes: a bison cannot become a predator because a decision model suggested `hunt`, while a raccoon can both forage and hunt only because simulation-owned configuration allows those actions.

## Implemented milestone: cross-domain organism forms

The composer now includes a simulation-owned organism-form module in addition to habitat, ecology, body/family, movement, capabilities and life history. `wild`, `domesticated` and `monster` are bounded species-form configurations rather than provider-authored labels. Their first functional semantics are intentionally small: deterministic settlement sensitivity and danger sensitivity feed coarse habitat suitability/carrying capacity.

Sheep demonstrates a domesticated-form bovid by reusing existing open-plains and large-grazer modules. It tolerates settlement pressure better than an otherwise comparable wild grazer, but there is still no owner ID, tame meter, command queue, breeding permission or inventory relationship; calling it `domesticated` does not fabricate those future systems. Warg demonstrates a monster-form canid predator by reusing existing forest/hills and predator modules, with lower danger sensitivity but stronger settlement aversion. Both species automatically enter the ordinary coarse/fine, predator graph, lineage/evolution, family-genome and decision pipelines.
...
## Implemented milestone: reusable wildlife movement controllers

Movement modes now execute as deterministic controller semantics rather than only speed/energy labels. Each movement archetype defines a gait plus bounded acceleration/deceleration, yaw turn rate and waypoint-arrival radius. A pure `wildlifeMovementController` step advances fine wildlife from current speed/heading toward a path waypoint; the game applies displacement only when the candidate position is not blocked. This gives heavy grazers slower acceleration and turning, dexterous foragers faster turning, and ambush/cursorial forms distinct controller envelopes while preserving inherited phenotype and family-genome locomotion modifiers.

Controller momentum is deliberately fine-runtime transient state. Position, needs, lineage, genetics and other authoritative individual state still persist; per-frame speed is reset on materialization/unload/transfer because distant chunks are simulated by coarse population/ecology rules rather than frozen fine physics. Decision Providers never select acceleration, heading or collision outcomes, and Jev does not need extra controller fields beyond existing movement-mode context.

## Implemented milestone: authoritative individual domestication

Mutable individual domestication now exists independently from immutable species form. A domesticated-form species is merely eligible; an individual sheep stores its own tame progress, owner ID, owner command, breeding permission, claim day and last interaction day. First-person feeding consumes conserved grain and advances taming by a deterministic bounded increment until the ownership threshold is reached. Owner commands execute in simulation rather than through Jev: `follow`, `stay` and `graze` bypass provider decisions, while `none` returns the animal to the ordinary bounded wildlife decision loop.

The God View boundary remains strict. A player-follow command is suspended whenever the player leaves first-person existence, and God-camera movement cannot become a follow target or move an owned animal. Provider-facing wildlife state is owner-anonymous: tame progress, command and breeding permission may be visible as read-only context, but the literal player owner ID is removed. This preserves the distinction between observing a historical ownership relation and making the player perceptible to simulated agents.

Breeding is authoritative as well. Unowned animals retain natural reproduction. Once ownership participates, both parents must share the same owner and both must have breeding enabled; the checks occur in legal action construction, mate targeting and conception. Permitted same-owner offspring inherit ownership while starting with breeding disabled. Durable lineage snapshots preserve inherited domestication at birth and terminal domestication at death, including the case where a father dies during gestation.
...
Terrain/contact and stateful-door phases are now implemented: bounded ground/slope traversal plus persistent semantic wall/threshold door state remain deterministic. The next major physics step is **rigid-body expansion** for movable/stackable bodies, carts/vehicles and projectile/contact events. Decision Providers remain restricted to intent while physics resolves actual motion/contact.
