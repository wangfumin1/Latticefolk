...
The next organism step is **authoritative domestication state**: owner/tame progress, command permissions, breeding/production relationships and transfer rules should be explicit mutable persisted state on eligible individuals, separate from immutable species `form`. Later physics integration can replace the current blocked-cell controller application without changing provider authority boundaries.
