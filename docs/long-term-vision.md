...
A species profile no longer has to be authored as one monolithic literal. `wildlifeArchetypes.ts` defines independently reusable habitat, ecology/trophic-niche, procedural-body and life-history modules plus a deterministic composer that produces an ordinary `WildlifeSpeciesProfile`. The composer copies module state into the species profile; all downstream simulation still consumes the same registry and therefore does not gain a second authority path.

Lynx is the first end-to-end composed species. It combines a temperate forest/hills habitat module, medium ambush-predator ecology, medium-felid body and solitary-predator life history, then supplies only its legal prey mapping. Adding it to the species registry automatically gives it coarse carrying capacity, migration, competition, disease pressure, predator pressure, fine materialization, reproduction, lineage/evolution statistics and family-genome handling. Fine rendering adds a generic `ear_tufts` morphology feature rather than a lynx-only mesh branch. Wolf↔lynx and lynx→prey legality remains defined by the shared predator graph.

The next organism step is **broader archetype coverage**: add more reusable habitat/ecology/body/life modules and composed herbivores, omnivores, predators, monsters and domesticated forms. After that, movement/animation mode and semantic interaction-capability modules can be split out while keeping generated content deterministic, bounded and conservation-safe.
