...
### Wildlife disease state

Coarse disease transmission state is embedded in each `CoarseChunkState` and therefore persists inside `coarse_chunks.state_json`: environmental reservoir, mean load, cross-species pressure, per-species pressure and strongest spillover. Coarse wildlife populations continue to persist their `diseaseLoad`. Lineage habitat JSON may now carry optional `diseasePressure`; lifetime habitat exposure time-weights that value when present. These are additive JSON fields, so existing SQLite schemas and older saves remain loadable without a table migration.
