...
Adding composed species such as lynx, bison, raccoon, sheep or warg follows the existing additive species-upgrade path: old coarse chunks gain newly configured species deterministically when simulated, while historical sparse pressure/source maps remain unknown for those species rather than being backfilled as observed zero. Movement/capability/form modules do not need persisted runtime multipliers because they are deterministic profile configuration; inherited individual locomotion variation remains in the existing organism genome. Fine movement-controller speed/heading are intentionally transient runtime values: persisted position remains authoritative, and rematerialization starts with zero controller momentum because unloaded chunks advance through coarse simulation rather than retaining per-frame velocity. Species `form` remains code-owned configuration and is never used as a substitute for ownership. Mutable individual domestication is persisted inside `WildlifeState.domestication`: fine individuals therefore round-trip through `fine_chunks.wildlife_json`, and identity-preserving transfers carry the same fields inside `wildlife_transfers.transfer_json`. The state contains tame progress, optional owner ID, owner command, breeding permission and claim/interaction dates.
