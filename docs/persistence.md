...
Reusable organism archetypes add no persistence schema of their own. A living/persisted individual still stores only its ordinary `species`, phenotype/genome and other `WildlifeState` fields; `archetypeId` and the species' habitat/ecology/body/life modules are code-owned registry configuration reconstructed from `WildlifeSpeciesProfile`. Adding a composed species such as lynx follows the existing additive species-upgrade path: old coarse chunks gain the newly configured species deterministically when simulated, while historical sparse pressure maps remain unknown for the new species rather than being backfilled as observed zero. No new third-party asset is required for lynx because its body uses procedural morphology.
