...
- deterministic fine movement controller。movement mode は speed/energy multiplier だけでなく acceleration、deceleration、visual turn rate、waypoint arrival radius を定義します。fine wildlife は pure deterministic controller で加速・制動・向き変更を行いますが、translation は pathfinder が承認した segment 上に固定されるため turn integration が blocked geometry を横切ることはありません。`moveSpeed/heading` は runtime transient state で、persisted `WildlifeState.position` が唯一の authoritative position です。
