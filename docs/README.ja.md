...
- multi-factor stability / generation-local evidence。対象種の generation を 1 つずつ除外して再推定し、pooled model と同じ feature set を保つ推定可能な refit だけで係数範囲と符号一貫性を要約します。各最近 generation endpoint では、その世代から過去へ広げ、既存の coverage / sample / variance / collinearity 条件を初めて満たす最狭ウィンドウを採用します。exposure は lifetime aggregate なので日単位履歴を捏造せず、異種間の generation 番号も同期しません。
