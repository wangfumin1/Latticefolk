...
- multi-factor interaction selection evidence。reproduction / offspring / lifespan ごとに complete-case モデルを作り、十分な coverage と分散を持つ predator / competition / disease source pressure を最大 6 特徴まで同時投入し、標準化 ridge（λ=0.25）で推定します。legacy unknown は 0 補完せず、サンプル不足や outcome 無分散は推定不能として表示します。God View の β*、coverage、R²、max|rX| は正則化された関連であり因果効果ではありません。
