...
- 确定性疾病传播：同种接触、跨种 spillover、环境 reservoir、湿地/降雨/密度共同形成 disease pressure；迁徙携带病压，fine 个体按附近动物距离和固定 spillover 权重暴露，God View 与 lineage lifetime exposure 可观察疾病压力。
...
当前顺序为：✅ coarse↔fine chunk 双向转换 → ✅ SQLite 世界持久化 → ✅ 跨 chunk 守恒流 → ✅ Region / World 决策层 → ✅ 动态 chunk streaming → ✅ 语义化程序聚落 → ✅ 生产链 → ✅ 第一版生态与生命周期 → ✅ durable ancestry / evolution statistics → ✅ 环境选择压力与 biome adaptation 可观测性 → ✅ 实际观测的 lifetime habitat exposure → ✅ 可保留个体身份的 fine migration / transfer → ✅ 生态位竞争 → ✅ 季节迁徙驱动 → ✅ 疾病传播 → **更多物种/捕食者与 fitness-by-habitat** → 更完整生态 → 完整物理层。
