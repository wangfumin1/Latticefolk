...
- deterministic fine movement controller：movement mode 现在不仅是 speed/energy 倍率，还定义 acceleration、deceleration、visual turn rate 与 waypoint arrival radius。fine wildlife 通过纯 deterministic controller 加速、制动和转向；实际平移仍严格沿 pathfinder 已批准的 segment，因此不会因为转向积分穿过 blocked geometry。`moveSpeed/heading` 只是 runtime transient state，存档中的 `WildlifeState.position` 仍是唯一 authoritative 位置。
