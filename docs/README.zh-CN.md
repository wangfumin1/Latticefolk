...
- multi-factor stability / generation-local evidence：对目标物种逐代执行 leave-one-generation-out 重拟合，只有 feature set 与 pooled model 完全一致的可估计结果才进入系数范围与符号一致性统计；近期每个 generation endpoint 从本代开始向前扩展，取第一个满足现有 coverage / sample / variance / collinearity 门槛的最窄窗口。当前 exposure 只有 lifetime aggregate，因此不会伪造逐日时间序列，也不跨物种同步 generation。
