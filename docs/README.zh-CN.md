...
- reusable generated organism archetypes：species profile 现在由 habitat、ecology/trophic niche、procedural body/family、movement mode、semantic wildlife capabilities、life-history 六类 simulation-owned module 组合。猞猁验证 felid 捕食者路径；野牛验证 open-plains large-grazer / bovid 路径；浣熊验证 forest-wetland opportunistic omnivore / procyonid 路径。统一 registry 自动接入 coarse ecology、fine materialization、movement cost/speed、合法行为、predator graph、family genome、lineage/evolution 与 God View；family 归属也已收回 species profile 作为单一来源。新增 ear tufts、face mask、ringed tail 都是通用 procedural morphology，没有新增第三方素材。
