...
The first runtime consequence consumer is fine wildlife predation. The same consequence-free contact gate now also protects first-person `chop` and `mine` resource mutations: the intended semantic object's authoritative collider must be the first reachable contact, so walls or materialized bodies can interpose before inventory/resource state changes. A provider may select the bounded `hunt` intention, but damage is applied only when `resolveContactAttack()` confirms the intended prey is the first contact within the fixed attack range. Static geometry, closed authoritative doors, movable bodies, the first-person player, NPCs, or other wildlife can therefore physically interpose. The resolver itself never mutates health or ecology state; deterministic wildlife simulation remains the owner of damage, hunger relief, kill/removal, and lineage outcome recording.
...
3. extend the now-runtime-used contact authority from wildlife attacks and first-person chop/mine tool contact into projectile mechanics and richer deterministic tool consequences;
