...
`wildlifeArchetypes.ts` now separates six reusable, simulation-owned modules: habitat affinity/seasonality, ecology and trophic niche, procedural body plus organism-family ownership, movement mode, semantic wildlife action capabilities, and life history. `composeWildlifeSpeciesProfile` copies those modules plus a species-specific legal prey map into the same `WildlifeSpeciesProfile` contract already consumed everywhere else. The profile is now the single source for organism family, movement semantics and legal base capabilities; `organismFamilies.ts` no longer keeps a duplicate species→family table.
