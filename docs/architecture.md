...
## Deterministic wildlife disease transmission

Wildlife disease is modeled as continuous deterministic pressure rather than random infection events. Coarse chunks track an environmental reservoir, mean population disease load, species-specific exposure pressure, aggregate cross-species pressure, and the strongest current spillover direction. Fixed simulation-owned spillover coefficients define how disease load from rabbit, deer, boar, and fox contributes to other species; same-species contact remains strongest.

Each coarse tick derives disease pressure from current species disease load, density, cross-species contact, environmental reservoir, wetland conditions and rain. Population disease load moves gradually toward that pressure, so disease can grow, spread across species, persist environmentally, or decay when pressure is removed. Mortality still reads the resulting deterministic disease load; no Decision Provider creates infection, severity, recovery or death.

Migration remains conservative while carrying disease burden. Coarse migrants blend source disease load into the destination population by transferred population weight; identity-preserving fine migrants already carry their own `diseaseLoad`. In fine simulation, nearby animals contribute exposure through distance-weighted spillover coefficients while the chunk reservoir contributes environmental pressure. The result updates the individual's disease load continuously and reproducibly.

Disease risk is visible but not authoritative in decision inputs. Current/neighbor migration candidates expose species-specific disease pressure, so fallback/Jev may choose a supplied safer adjacent chunk, while adjacency, capacity, population amount and physical transfer remain deterministic. Lineage birth/death habitat snapshots and observation-bounded lifetime exposure now retain disease pressure alongside competition and seasonal suitability, enabling later fitness-by-habitat analysis.
