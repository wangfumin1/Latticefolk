...
A single pooled multi-factor model can still hide cohort dependence or regime changes. The evolution layer therefore derives deterministic sensitivity evidence from the same target-species lineage without introducing random resampling or causal claims. For each outcome, it removes one target-species generation at a time and reruns the complete selection/coverage/collinearity pipeline. A replicate is coefficient-comparable only when it remains estimable with exactly the same feature set as the pooled model. Comparable replicates expose coefficient mean/range and sign consistency; sign consistency is reported only after at least three comparable refits. This is leave-one-generation-out sensitivity, not a confidence interval or p-value.
