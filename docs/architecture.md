...
Movement and capability modules are functional configuration, not labels. Fine locomotion multiplies the inherited phenotype/genome trade-offs by the profile's bounded movement speed/energy/fast-action factors. Movement profiles additionally define acceleration, deceleration, visual turn rate and waypoint arrival radius; `stepWildlifeMovementController` applies those values deterministically. `wildlifeAllowedActions` starts from the profile capability set and then applies deterministic state gates for maturity, pregnancy, migration eligibility and predator legality before a request reaches fallback/Jev. Providers therefore cannot request `hunt` for a bison or invent an unregistered capability. A composed ecology module may define legal trophic role and prey edges as static game configuration, but neither Jev nor another Decision Provider can create or mutate those rules, family assignment, movement multipliers or outcomes at runtime. God View exposes family, archetype, organism form, movement mode/controller state and capabilities for inspection without affecting simulation or chunk generation.

### Fine movement controller boundary

The first reusable movement controller intentionally stops short of a physics replacement. The pathfinder still supplies legal waypoint segments and translation remains collinear with the current segment. The controller only ramps scalar speed, computes stopping speed before the waypoint radius, clamps per-frame `dt`, and limits visual yaw by the movement archetype's `turnRate`. This gives cursorial, ambush, sturdy, heavy-grazer and dexterous-forager modes observable movement dynamics without allowing a turn integrator to cut through blocked geometry.

`WildlifeRuntime.moveSpeed` and `heading` are transient integration state. `WildlifeState.position` remains the authoritative fine-world position and is the only movement state persisted/folded into the world. On materialization/restart the controller resumes from zero speed at the persisted position; this may change a few frames of visual acceleration but cannot change inventory, resources, population, lineage or the saved location. Decision Providers never receive numerical controller authority: they choose a legal intent/target, while deterministic pathfinding and the controller resolve movement.
