...
Time locality is deliberately bounded by what is actually persisted. `habitat_exposure_json` contains lifetime aggregate source means, not an exact within-lifetime exposure time series, so the system does **not** invent calendar-day local pressure histories. Instead it uses the target species' own generations as the temporal basis. For each of the latest generation endpoints, it starts with that generation and expands backward until the first estimable multi-factor model is found. The resulting window is therefore the narrowest generation-local window supported by coverage, sample-size, outcome-variance and collinearity rules. Windows may overlap and remain descriptive observational evidence; generation numbers are never aligned across species.
