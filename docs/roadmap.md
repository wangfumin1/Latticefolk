...
- 🚧 Physics authority v2: ✅ terrain/ground contact + bounded slopes + home/fine-chunk terrain sleeping; ✅ persistent semantic building doors with wall/threshold decomposition, occupied-threshold safety, real open/save-reload/traverse/close/block Chromium coverage, and God View observer regression coverage. Next: general rigid bodies for movable props/carts/stacking, then projectile/contact queries. Keep unloaded chunks asleep under coarse simulation and all physical outcomes outside Decision Providers.
