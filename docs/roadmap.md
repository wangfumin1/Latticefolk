...
- ✅ First reusable fine movement controller: movement modes now define bounded acceleration, braking, visual turn rate and waypoint radius; a pure deterministic controller applies them while translation stays on pathfinder-approved segments and persisted position remains authoritative.
- ⏭ Extend movement execution with reusable gait/animation/controller hooks and eventual physics-backed contact/terrain semantics; separately add authoritative domestication ownership/taming/commands as persisted individual state rather than overloading species form.
