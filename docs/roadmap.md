...
- 🚧 Physics authority v2: terrain/ground contact, bounded slopes, authoritative home/fine-chunk terrain, chunk-sleep teardown, and the generic invisible door-collider foundation are implemented. The first movable-prop runtime now promotes the existing licensed town cart into a persisted physics-resolved body that characters must avoid and the first-person player can push. Next: reusable rigid-body archetypes, stacking/support, projectile/contact queries, and richer sleeping/wakeup. Do not add separate visible door meshes; authored building assets already contain their doors/entrances.
