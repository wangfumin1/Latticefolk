...
- 🚧 Physics authority v2: terrain/ground contact, bounded slopes, runtime fine-chunk terrain registration, and chunk-sleep teardown are implemented in the current increment with playable-browser coverage. Next: authoritative door/open-close collider state, then general rigid bodies for movable props/carts/stacking and projectile/contact queries. Keep unloaded chunks asleep under coarse simulation and all physical outcomes outside Decision Providers.
