...
- ⏭ Authoritative domestication state: add explicit per-individual ownership/taming/command/breeding semantics and persistence for domesticated-capable organisms, keeping species `form` separate from mutable individual domestication state; then continue toward dedicated physics/character controllers.
