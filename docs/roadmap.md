...
- 🚧 Decision batching, event-driven re-decisions, spatial partitioning, simulation LOD, and performance budgets. **Coarse↔fine materialization, durable persistence, pressure/surprise-driven adaptive chunk decision scheduling, bounded rotating discovered-world candidate scans, a persistent coarse-chunk coordinate index reused by flow/ecology neighbor planning, and the first deterministic fine-physics spatial hash are implemented; broader simulation LOD and performance-budget work remain.**
...
- 🚧 Physics authority v2: terrain/ground contact, bounded slopes, authoritative home/fine-chunk terrain, chunk-sleep teardown, the generic invisible door-collider foundation, deterministic segment/swept-circle contacts, contact-gated fine predation, and the first persisted rigid-body archetype are implemented. The licensed town cart now persists canonical `rigidBodyArchetype` semantics and resolves collision parameters through a shared registry, with legacy movable snapshots upgraded on load. Next: stacking/support, additional licensed movable archetypes, projectile/tool consequences on the same contact authority, and richer sleeping/wakeup. Do not add separate visible door meshes; authored building assets already contain their doors/entrances.
