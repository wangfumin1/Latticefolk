...
- 🚧 Physics authority v2: terrain/ground contact, bounded slopes, authoritative home/fine-chunk terrain, chunk-sleep teardown, the generic invisible door-collider foundation, the persisted movable town cart, and deterministic segment/swept-circle contact queries are implemented. Fine wildlife `hunt` is the first real consequence consumer: damage occurs only when the intended prey is the first authoritative contact, so static geometry, closed doors and materialized bodies can interpose without giving the Decision Provider physical authority. Next: reusable rigid-body archetypes, stacking/support, projectile/tool consequences on the same contact authority, and richer sleeping/wakeup. Do not add separate visible door meshes; authored building assets already contain their doors/entrances.
