...
- ⏭ Reusable movement/controller execution: turn movement modes into explicit controller/animation semantics (gait, acceleration/turn constraints, sleep/unloaded behavior), then add authoritative domestication ownership/taming/commands as separate persisted state rather than overloading species form.
