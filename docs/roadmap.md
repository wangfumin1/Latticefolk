...
- 🚧 Physics authority v2: terrain/ground contact, bounded slopes, authoritative home/fine-chunk terrain, chunk-sleep teardown, the generic invisible door-collider foundation, the persisted movable town cart, and deterministic segment/swept-circle contact queries are implemented. Contact queries share the same static/closed-door authority and caller-supplied materialized dynamic circles with stable nearest-hit ordering. Next: reusable rigid-body archetypes, stacking/support, deterministic projectile/tool consequences on top of contact queries, and richer sleeping/wakeup. Do not add separate visible door meshes; authored building assets already contain their doors/entrances.
