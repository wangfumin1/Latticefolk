...
- 🚧 Decision batching, event-driven re-decisions, spatial partitioning, simulation LOD, and performance budgets. **Coarse↔fine materialization, durable persistence, pressure/surprise-driven adaptive chunk decision scheduling, bounded rotating discovered-world candidate scans, and the first deterministic fine-physics spatial hash are implemented; broader entity/world spatial partitioning and performance-budget work remain.**
