...
- Physics authority v2 terrain + door increments: authoritative ground/slope/step legality is joined by semantic building wall decomposition and persistent stateful door colliders. `WorldObjectState.doorOpen` uses existing object persistence, closed doors block the shared kinematic authority, open doors release the threshold, closing onto an occupying body is rejected, and real Chromium E2E covers open → save/reload → traverse → close → block plus God View observer invariants.
