    ...
    const cards=active.map(entry=>`
      <div class="evo-card">
        ...
        ${entry.multifactorSelection.stability.filter(stability=>stability.leaveOneGenerationOut.attemptedReplicates>=2||stability.localWindows.some(window=>window.model.estimable)).map(stability=>`
          <div class="evo-selection">
            <b>${i18n.t('evolution.stability')} · ${i18n.t(`evolution.outcome.${stability.outcome}`)}</b> · LOO-G ${stability.leaveOneGenerationOut.comparableReplicates}/${stability.leaveOneGenerationOut.estimableReplicates}/${stability.leaveOneGenerationOut.attemptedReplicates}
