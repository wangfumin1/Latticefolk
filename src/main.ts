  ...
  renderEvolutionPanel() {
    ...
    const cards=active.map(entry=>`
      <div class="evo-card">
        ...
        ${entry.phenotype.mean?`<div class="evo-traits"><b>${i18n.t('evolution.phenotype')}</b> n=${entry.phenotype.sampleSize} · ${i18n.t('evolution.morphology')} L ${trait(entry.phenotype.mean.morphology.bodyLength)} H ${trait(entry.phenotype.mean.morphology.bodyHeight)} leg ${trait(entry.phenotype.mean.morphology.legLength)} head ${trait(entry.phenotype.mean.morphology.headScale)} tail ${trait(entry.phenotype.mean.morphology.tailScale)}</div>
