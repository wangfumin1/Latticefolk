...
export function fallbackWildlifeDecisions(req: WildlifeDecisionBatchRequest): WildlifeDecisionBatchResponse {
  const decisions:WildlifeDecisionResult[]=req.requests.map(entry=>{
    ...
    const migrationPressure=currentHabitat.density>=.9||currentHabitat.competitionPressure>=65||currentHabitat.diseasePressure>=62||currentHabitat.seasonalSuitability<42||currentHabitat.ecology<38||currentHabitat.water<30||currentHabitat.food<34||currentHabitat.danger>70;
