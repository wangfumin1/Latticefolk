    ...
    const state={
      ...
      animals:requests.map(entry=>({
        ...
        domestication:entry.wildlife.domestication?{
          stage:entry.wildlife.domestication.stage,
          command:entry.wildlife.domestication.command,
          ownerKind:entry.wildlife.domestication.ownerKind
        }:undefined,
      ...
      authority:'Select behavior and supplied targets only. Phenotype, organism-family genome, organism form, movement mode, capabilities and domestication state are simulation-owned read-only context. Never directly mutate health, needs, reproduction, population, resources, phenotype, organism genome, organism form, capabilities, ownership, taming progress, commands, physiology, or genetics.'
    };
