...
- **Bounded organism forms**: `wild`, `domesticated`, and `monster` are simulation-owned form archetypes. They currently affect deterministic coarse settlement sensitivity and danger sensitivity, are visible in God View, and are supplied to Jev only as read-only context. `domesticated` does **not** yet mean an individual has an owner, tame state, commands, breeding rights, or inventory; those require a later authoritative domestication system.
- **Deterministic fine movement controllers**: movement archetypes now define bounded acceleration, braking, visual turn rate, and waypoint arrival radius in addition to speed/energy multipliers. Fine wildlife accelerates and brakes through a pure deterministic controller while translation remains constrained to pathfinder-supplied segments, so this stage cannot cut corners through blocked geometry. Controller speed/heading are transient runtime state; persisted `WildlifeState.position` remains the authoritative fine position.
